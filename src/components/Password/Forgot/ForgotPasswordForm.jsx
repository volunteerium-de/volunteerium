...
const ForgotPasswordForm = (props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (props.onSubmit) {
        props.onSubmit(); // Calls the onSubmit function passed via props
      }
    },
  });

  return (
    <form onSubmit={formik.handleSubmit} className="flex flex-col items-start space-y-6 p-6 w-full max-w-[44.1875rem] bg-white dark:bg-black rounded-lg relative">
      ...
      <div className="w-full">
        ...
        <p className="text-left w-full text-[1rem] md:text-[1.125rem] font-normal text-gray-2 dark:text-white leading-snug">
          Don't worry, this can happen to anyone. To reset your account password, we will send you a verification code via email.
        </p>
      </div>
      ...
      <button
        type="submit"
        className="bg-primary-green text-white py-4 w-full max-w-[44.1875rem] h-[2.8125rem] rounded-lg hover:bg-dark-green transition duration-300"
      >
        Submit
      </button>
      ...
      <p className="mt-4 text-center text-sm dark:text-white w-full">
        ...
        <span className="text-primary-green cursor-pointer underline">
          Sign Up
        </span>
      </p>
    </form>
