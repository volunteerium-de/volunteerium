...
const VerificationForm = ({ onSubmit, onResend }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit(code.join('')); // Call the `onSubmit` function passed from props
  };

  return (
    <form onSubmit={handleSubmit} className="flex flex-col items-start space-y-6 p-6 w-full max-w-[44.18rem] bg-white dark:bg-black rounded-lg relative">
      ...
      >
        <IoIosArrowBack className="text-2xl text-black dark:text-white" /> {/* Light mode: `text-black`, Dark mode: `dark:text-white` */}
        <span className="text-lg font-semibold text-black dark:text-white">Back to login</span> {/* Light mode: `text-black`, Dark mode: `dark:text-white` */}
      </div>
      ...
      <div className="text-center mb-4 w-full">
        <span className="text-danger text-[1.125rem] font-semibold">
          {formatTime(timeLeft)}
        </span>
      </div>
      ...
      <button
        type="submit"
        className={`py-4 w-full max-w-[44.18rem] h-[2.8125rem] rounded-lg transition duration-300
      ...
      <p className="text-center text-sm mt-4 text-gray-3 dark:text-white w-full">
        {timerMessage}{' '}
        <span className="text-primary-green cursor-pointer underline" onClick={onResend}>
          Resend
        </span>
      </p>
    </form>
