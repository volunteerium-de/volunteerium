    ...
    <>
      {loading ? (
            ...
            <thead className="thead">
              <tr className="tr w-full bg-primary-green dark:bg-light-gray text-light-gray dark:text-dark-gray-1 uppercase text-sm leading-normal">
                <th className="th p-3 text-left">
                  {t(translations.adminPanel.users.usersTable.userId)}
                </th>
                <th className="th p-3 text-center">
                  {t(translations.adminPanel.users.usersTable.status)}
                </th>
                <th className="th p-3 text-center">
                  {t(translations.adminPanel.users.usersTable.userType)}
                </th>
                <th className="th p-3 text-left">
                  {t(translations.adminPanel.users.usersTable.nameOrg)}
                </th>
                <th className="th p-3 text-left">
                  {t(translations.adminPanel.users.usersTable.email)}
                </th>
                <th className="th p-3 text-center">
                  {t(translations.adminPanel.users.usersTable.createdAt)}
                </th>
              </tr>
            </thead>
        ...
        <div className="p-4 sm:p-0 mt-4 text-center sm:text-left text-dark-gray-2 dark:text-light-gray">
          {t(translations.adminPanel.users.usersTable.noUsersFound)}
        </div>
      )}
    </>
  )
